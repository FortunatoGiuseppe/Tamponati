<template>
  <q-page class="q-pa-md">
    <div class="text-center" style="font-size: 1.2rem"><q-icon name="admin_panel_settings" size="md" /><br />Amministratore</div>
    <q-separator />
    <div class="q-pa-md">
        <div class="q-gutter-xs">
            <q-chip removable v-model="medici" @remove="faQualcosa()" color="primary" text-color="white" icon="medical_services">
                    Medici
            </q-chip>
    ...
    <q-list  class="rounded-borders relative-position" bordered separator>
        ...
        <transition-group name="scale" move-class="move-transition">
            <template v-for="richiesta in richiesteMed" v-bind:key="richiesta.id">
                <q-item v-ripple clickable>
                    ...
                    <q-item-section class="col-2 text-black" side>
                        <q-item-label><q-btn icon="verified_user" flat @click="approvaRichiesta(richiesta)" /></q-item-label>
                    </q-item-section>
                </q-item>
            </template>
        </transition-group>
    ...
    <q-list class="rounded-borders relative-position" bordered separator>
        ...
        <transition-group name="scale" move-class="move-transition">
            <template v-for="richiesta in richiesteLab" v-bind:key="richiesta.id">
                <q-item v-ripple clickable>
                    <q-item-section>
...
export default defineComponent({
  ...
  created () {
      db.collection('users') .where('tipo_utente', '==', 2).get().then
      (querySnapshot => {
      ...
      db.collection('users') .where('tipo_utente', '==', 4).get().then
      (querySnapshot => {
          querySnapshot.forEach(doc => {
              const data = {
                  'id': doc.id,
